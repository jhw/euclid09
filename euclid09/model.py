...
"""
Samplers are unique because their sounds have to be provided from external sources, they aren't internally generated
A track is bound to a machine; when a track is rendered, the machine is instantiated and notes generated by "playing" a machine with user- defined generators
Sampler- based machines require sounds and cutoff parameters; these are stored at the track level and passed to the machine upon initialisation
Hence a sampler based machine needs its own track class
All pool- and tag- related stuff should be embedded in this subclass, as this is where the sampler gets its sounds from
"""
